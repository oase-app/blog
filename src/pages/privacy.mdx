...
Some data **transits** non-EU services: push notifications are delivered via Apple, Google, and Expo (US), but their content is encrypted and only decrypted on your device. Technical error reports go to Sentry (US) with personal identifiers scrubbed before sending. Application logs — which never contain your content, since it is encrypted before it reaches our servers — are streamed to Logflare (US) and deleted automatically after 7 days. Application performance telemetry is processed by New Relic in its EU region.
...
| Logflare (Supabase, Inc.), US | Application logs (no content), deleted after 7 days |
| New Relic, Inc., US | Performance monitoring (technical telemetry), EU region |
